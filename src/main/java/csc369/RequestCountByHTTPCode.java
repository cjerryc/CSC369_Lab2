...
public class RequestCountByHTTPCode {
    // ----- Job 1 -----
    public static final Class OUTPUT_KEY_CLASS_1 = Text.class;
    public static final Class OUTPUT_VALUE_CLASS_1 = IntWritable.class;
    //    Mapper 1 to Count Requests by HTTP code (Mapper is called once every line of the input text file)
    public static class Mapper1 extends Mapper<LongWritable, Text, Text, IntWritable> {
        private final static IntWritable one = new IntWritable(1);
        private Text HTTPCode = new Text();
    ...
    // Reducer 1 to Count Requests by HTTP Code (Hadoop groups the output from Mapper by unique keys, so when the reducer is applied
    ...
    public static class Mapper2 extends Mapper<LongWritable, Text, IntWritable, Text> {
        ...
        protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if (parts.length == 2) {
                HTTPCode.set(parts[0]);
                count.set(Integer.parseInt(parts[1]));
                context.write(count, HTTPCode);  // set (key, value) pair as (count, HTTP response code)
            }
        }
    }
