...
public class RequestCountByURL {
    // ----- Job 1 -----
    public static final Class OUTPUT_KEY_CLASS_1 = Text.class;
    public static final Class OUTPUT_VALUE_CLASS_1 = IntWritable.class;
    //    Mapper 1 to Count Requests by URl (Mapper is called once every line of the input text file)
    public static class Mapper1 extends Mapper<LongWritable, Text, Text, IntWritable> {
        ...
        protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if (parts.length > 6) {
                String path = parts[6];  // parts[6] is the URL portion
                urlPath.set(path);
                context.write(urlPath, one);  // set (key, value) pair as (URL, count)
            }
        }
    }
    // Reducer 1 to Count Requests by URl (Hadoop groups the output from Mapper by unique keys, so when the reducer is applied
    ...
    public static class Mapper2 extends Mapper<LongWritable, Text, IntWritable, Text> {
        ...
        protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if (parts.length == 2) {
                ...
                context.write(count, path);  // set (key, value) pair as (count, URL)
            }
        }
    }
    ...
    public static class Reducer2 extends Reducer<IntWritable, Text, IntWritable, Text> {
        ...
        protected void reduce(IntWritable key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
            for (Text v : values)
                context.write(key, v);  // (count, path)
        }
    }
